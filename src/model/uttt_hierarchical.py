...
@dataclass
class UTTTHierarchicalTransformerConfig:
...
class UTTTHierarchicalTransformer(nn.Module):
    def __init__(self, config: UTTTHierarchicalTransformerConfig) -> None:
        super().__init__()
    ...
    def forward(self, x: torch.LongTensor, y: torch.LongTensor) -> torch.Tensor:
        ...
        h_meta = h_meta + self.meta_pos_emb(self.positions)[None, :, :] + self.active_board_emb(y)
        h_meta = torch.cat(
